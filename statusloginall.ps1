<#
  ...
  Purpose:
    ...
    - Output a table; optional CSV; keep console open at the end.
...
param(
  ...
  [string]$OutCsv     = ""                 # optional CSV path
)

# --- Import AD module (fail fast) ---
try { Import-Module ActiveDirectory -ErrorAction Stop }
catch { Write-Error "ActiveDirectory (RSAT) missing or not running in PS7."; exit 1 }
...
function Invoke-QuserSafe {
  ...
  $sb = { param($c) try { quser /server:$c 2>&1 | ForEach-Object { $_.ToString() } } catch { @() } }
  ...
  try {
    ...
    if ($job -and $job.State -ne 'Removed') { Remove-Job $job -Force -ErrorAction SilentlyContinue | Out-Null }
  }
}
  ...
  foreach ($ln in $payload) {
    ...
    if ($parts.Count -ge 6) {
      ...
      foreach ($fmt in @([System.Globalization.CultureInfo]::InvariantCulture, (Get-Culture))) {
        try { $logon = [datetime]::Parse($logonStr, $fmt); break } catch {}
      }
    }
...
function Resolve-Login {
  ...
  if ([string]::IsNullOrWhiteSpace($RawUser)) { return [pscustomobject]@{ Login=''; User=''; Note='empty' } }

  $candidate = $RawUser
  if ($RawUser -like '*\*') { $null, $candidate = $RawUser -split '\\', 2 }
...
function Get-PreciseLogonTime {
  ...
  try {
    ...
    if (-not $acct) { $sess | Remove-CimSession -ErrorAction SilentlyContinue; return $null }
    ...
    if (-not $assoc) { $sess | Remove-CimSession -ErrorAction SilentlyContinue; return $null }

    # Collect times without using a pipe after foreach (avoids parser issues).
    $times = @()
    foreach ($s in $assoc) {
      try {
        # Some CIMs might not be Win32_LogonSession objects; check properties safely.
        if ($s.PSObject.Properties.Match('LogonType').Count -gt 0 -and $s.PSObject.Properties.Match('StartTime').Count -gt 0) {
    ...
    if ($times.Count -gt 0) { ($times | Sort-Object -Descending | Select-Object -First 1) } else { $null }
  } catch { $null }
}
...
foreach ($pc in $TargetComputers) {
  ...
  if ($null -eq $raw) {
    ...
    $Rows.Add([pscustomobject]@{ Computer=$pc; Room=$room; Floor=$floor; Login=''; User=''; LogonTime=''; Note='no response/timeout' }) | Out-Null
    continue
  }
  ...
  if (-not $entries -or $entries.Count -eq 0) {
    ...
    $Rows.Add([pscustomobject]@{ Computer=$pc; Room=$room; Floor=$floor; Login=''; User=''; LogonTime=''; Note='no logged users' }) | Out-Null
    continue
  }

  # Resolve and print inline as "login [yyyy-MM-dd HH:mm:ss]"
  $inlineBits = @()
  foreach ($e in $entries) {
    ...
    if ($logon) { $inlineBits += ("{0} [{1:yyyy-MM-dd HH:mm:ss}]" -f $r.Login, $logon) }
    else { $inlineBits += $r.Login }
  }
...
$Rows | Sort-Object Floor, Room, Computer, Login |
