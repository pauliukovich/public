<#
  ...
  Purpose:
    ...
    - Additionally write a readable text report to C:\Windows\database\online\online-users.txt
...
# --- Text report to C:\Windows\database\online\online-users.txt ---
$OutTxt = 'C:\Windows\database\online\online-users.txt'
$txtDir = Split-Path $OutTxt -Parent
if (-not (Test-Path $txtDir)) {
  New-Item -ItemType Directory -Path $txtDir -Force | Out-Null
}

$sb = New-Object System.Text.StringBuilder
$null = $sb.AppendLine("===== ONLINE USERS REPORT =====")
$null = $sb.AppendLine("Generated : $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')")
$null = $sb.AppendLine()

# Group rows by computer so that each machine has its own block
$groupedByComputer = $Rows | Sort-Object Floor, Room, Computer, Login | Group-Object Computer, Room, Floor

foreach ($g in $groupedByComputer) {
  $first = $g.Group | Select-Object -First 1
  $pc    = $first.Computer
  $room  = $first.Room
  $floor = $first.Floor

  $null = $sb.AppendLine("Computer : $pc")
  if ($room -ne $null -and $room -ne '') {
    $null = $sb.AppendLine("Room     : $room")
  }
  $null = $sb.AppendLine("Floor    : $floor")

  $rowsForPc = $g.Group
  $userRows  = $rowsForPc | Where-Object { $_.Login -ne '' }

  if (-not $userRows -or $userRows.Count -eq 0) {
    $note = ($rowsForPc | Select-Object -First 1).Note
    if (-not $note) { $note = 'no data' }
    $null = $sb.AppendLine("Status   : $note")
    $null = $sb.AppendLine()
  } else {
    foreach ($r in $userRows) {
      $displayUser = if ($r.User -and $r.User -ne $r.Login) {
        "$($r.User) ($($r.Login))"
      } else {
        $r.Login
      }
      $null = $sb.AppendLine("  User   : $displayUser")
      if ($r.LogonTime -and $r.LogonTime -ne '') {
        $null = $sb.AppendLine("  Logon  : $($r.LogonTime)")
      }
      if ($r.Note) {
        $null = $sb.AppendLine("  Note   : $($r.Note)")
      }
      $null = $sb.AppendLine()
    }
  }
...
Write-Host "Text report written to: $OutTxt" -ForegroundColor Cyan
